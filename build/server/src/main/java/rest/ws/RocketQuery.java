...
public class RocketQuery extends AbstractRocketQuery {
  ...
  protected QueryResult executeOperation(
    ...
    switch (query) {
      ...
      case "getTestModelById":
        {
          OutObject one = gqlToSql.execute("TestModel", field, ctx.readLong());
          if (subscribed) {
            OutObjectTracker tracker = new OutObjectTracker(dataChangeTracker, session, field);
            tracker.init(one);
            return singleResult("TestModel", false, one, tracker);
          }
          return singleResult("TestModel", false, one);
        }
    }
    D3ELogger.info("Query Not found");
