...
@Service("TestModel")
public class TestModelEntityHelper<T extends TestModel> implements EntityHelper<T> {
  ...
  public void fromInput(T entity, GraphQLInputContext ctx) {
    if (ctx.has("number")) {
      entity.setNumber(ctx.readInteger("number"));
    }
  ...
  public T getById(long id) {
    return id == 0l ? null : ((T) testModelRepository.findById(id));
  }
