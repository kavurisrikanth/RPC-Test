...
public class D3ESubscription implements FlowableOnSubscribe<DataStoreEvent> {
  ...
  public Flowable<D3ESubscriptionEvent<TestModel>> onTestModelChangeEvent() {
    return this.flowable
        .filter((e) -> e.getEntity() instanceof TestModel)
        .map(
            (e) -> {
              D3ESubscriptionEvent<TestModel> event = new D3ESubscriptionEvent<>();
              event.model = ((TestModel) e.getEntity());
              event.changeType = e.getType();
              return event;
            });
  }
